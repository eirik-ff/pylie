...
def test_construct_identity_with_no_args():
    ...
    np.testing.assert_equal(se3.translation, np.zeros((3,)))


def test_construct_with_tuple():
    so3 = SO3() + np.array([0.1, -0.3, 0.2])
    t = np.array([1, 2, 3])
    se3 = SE3(so3, t)
...
def test_composition_with_identity():
    X = SE3(SO3.from_roll_pitch_yaw(np.pi / 10, -np.pi / 3, 3 * np.pi / 4), np.array([3, 2, 1]))
...
def test_composition():
    X = SE3(SO3.rot_x(np.pi / 2), np.array([1, 2, 3]))
    Y = SE3(SO3.rot_z(np.pi / 2), np.array([0, 1, 0]))

    Z = X.compose(Y)

    rot_expected = SO3.from_angle_axis(2 * np.pi / 3, np.array([1, -1, 1]) / np.sqrt(3))
    t_expected = np.array([1, 2, 4])
...
def test_composition_with_operator():
    X = SE3(SO3.rot_x(3 * np.pi / 2), np.array([1, 2, 3]))
    Y = SE3(SO3.rot_y(np.pi / 2), np.array([-1, 0, 1]))

    Z = X @ Y

    rot_expected = SO3.from_angle_axis(2 * np.pi / 3, np.array([-1, 1, -1]) / np.sqrt(3))
    t_expected = np.array([0, 3, 3])
...
def test_inverse():
    X = SE3(SO3.from_roll_pitch_yaw(np.pi / 4, np.pi / 2, 3 * np.pi / 2), np.array([1, -2, 3]))
...
def test_action_on_vectors():
    ...
    X = SE3(SO3.from_angle_axis(np.pi / 2, np.array([0, 1, 0])), t)
...
def test_action_on_vectors_with_operator():
    ...
    vec_expected = np.array([0, 3, 3])

    np.testing.assert_almost_equal(X * x, vec_expected, 14)


def test_hat():
    rho_vec = np.array([1, 2, 3])
    theta_vec = np.pi / 3 * np.array([1, -1, 2]) / np.sqrt(6)
    xi_vec = np.hstack((rho_vec, theta_vec))

    xi_hat_expected = np.block([[SO3.hat(theta_vec), rho_vec[:, None]],
                                [np.zeros((1, 4))]])
...
def test_vee():
    rho_vec = np.array([4, 3, 2])
    theta_vec = 2 * np.pi / 3 * np.array([2, 1, 1]) / np.sqrt(6)
    xi_vec = np.hstack((rho_vec, theta_vec))
...
def test_log_with_no_rotation():
    ...
    # Rotation part:
    np.testing.assert_equal(xi_vec[3:], np.zeros((3,)))


def test_exp_with_no_rotation():
    rho_vec = np.array([1, 2, 3])
    theta_vec = np.zeros((3,))
    xi_vec = np.hstack((rho_vec, theta_vec))
...
def test_oplus_with_ominus_diff():
    X = SE3(SO3.from_roll_pitch_yaw(np.pi / 10, -np.pi / 3, 3 * np.pi / 4), np.array([3, 2, 1]))
    Y = SE3(SO3.from_roll_pitch_yaw(np.pi / 7, np.pi / 3, 4 * np.pi / 6), np.array([2, 1, 0]))
...
def test_ominus_with_oplus_diff_with_operators():
    X = SE3(SO3.from_roll_pitch_yaw(np.pi / 7, np.pi / 3, 4 * np.pi / 6), np.array([2, 1, 0]))
...
def test_jacobian_inverse():
    ...
    delta = 1e-3 * np.ones((6,))
    taylor_diff = X.oplus(delta).inverse() - X.inverse().oplus(J_inv @ delta)
    np.testing.assert_almost_equal(taylor_diff, np.zeros((6,)), 14)


def test_jacobian_action_Xx_wrt_X():
    X = SE3(SO3.from_roll_pitch_yaw(np.pi / 8, np.pi / 2, 5 * np.pi / 6), np.array([2, 1, 1]))
    x = np.array([1, 2, 3])
...
def test_jacobian_right():
    ...
    np.testing.assert_almost_equal(taylor_diff, np.zeros((6,)), 5)


def test_jacobian_left():
    rho_vec = np.array([2, 1, 2])
...
def test_jacobian_X_oplus_tau_wrt_tau():
    ...
    np.testing.assert_almost_equal(taylor_diff, np.zeros((6,)), 6)
